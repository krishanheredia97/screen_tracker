...
class DataLogger:
    ...
    def _initialize_log_file(self):
        if not os.path.exists(self.log_file):
            with open(self.log_file, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["datetime_start", "datetime_end", "duration_seconds", "window_title", "user_state"])
    ...
    def log_window_activity(self, start_time, end_time, window_title, user_state):
        with self.lock:
            ...
            print(f"Logging: {start_str}, {end_str}, {duration:.0f}, {window_title}, {user_state}")
            with open(self.log_file, 'a', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow([start_str, end_str, f"{duration:.0f}", window_title, user_state])
                f.flush()
